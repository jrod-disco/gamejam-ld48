...
import { APP_HEIGHT, APP_WIDTH, MAX_LAYER_DEPTH } from '@src/constants';
 ...
 * @param depth    relative to MAX_LAYER_DEPTH
 */
export const positionUsingDepth = (
    obj: DisplayObject, x: number, y: number, depth: number
): void => {
    obj.position.x = APP_WIDTH/2 + ((APP_WIDTH/2-x)/MAX_LAYER_DEPTH) * depth;
    obj.position.y = APP_HEIGHT/2 + ((APP_HEIGHT/2-y)/MAX_LAYER_DEPTH) * depth;
}